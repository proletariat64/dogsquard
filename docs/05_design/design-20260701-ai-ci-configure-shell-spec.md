...
Optional live activation may be added through an explicit flag:

```bash
scripts/configure-ai-ci.sh --apply --apply-github-vars
```

This flag may set non-secret GitHub repository variables, but must never create, read, print, or store secret values.
...
The script may print commands or perform them directly only when:
...
Secrets are never set by this script. The script prints required secret names only.
...
Use the user-provided playground:
...
Known prepared secrets in the UAT repo:
