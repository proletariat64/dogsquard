...
prepare_prompt() {
...

  {
    ...
    echo "3. Use explicit GitNexus repository scope for this repo. In multi-repo indexed environments, unscoped GitNexus CLI commands can fail or inspect ambiguous graph context."
    echo "   npx gitnexus query --repo dogsquard \"<concept or error>\""
    echo "4. Before editing any existing function, class, or method, run:"
    ...
    echo "5. If GitNexus impact is HIGH or CRITICAL, stop and report instead of editing."
    echo "6. If gitnexus analyze or index refresh fails, disclose the failure in issue/PR evidence and treat later GitNexus output as incomplete evidence."
    echo "7. Implement the smallest practical safe fix."
    echo "8. Add or update a focused regression test when practical."
    echo "9. Run validation commands appropriate to the changed files."
    echo "10. Run:"
    echo "   npx gitnexus detect-changes --repo dogsquard > .tmp/ai-fix/gitnexus-detect-changes.txt"
    echo "11. If you cannot safely finish, do not leave partial code changes."
    echo
