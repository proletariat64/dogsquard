...
Environment:
  ...
  ACTION                  start|stop|status|restart|health. Defaults to status.
...
if [[ "$HOST" == "us.hermes" && ( "$ACTION" == "start" || "$ACTION" == "restart" ) && "$ALLOW_US_HERMES_RUNTIME" != "true" ]]; then
  echo "Blocked: runtime ${ACTION} on us.hermes requires ALLOW_US_HERMES_RUNTIME=true." >&2
  echo "Phase 6B-3 keeps us.hermes dry-run/documentation only to protect multica routing." >&2
  exit 1
fi
...
ssh "$HOST" \
  "APP_NAME=$(quote "$APP_NAME") DEPLOY_ROOT=$(quote "$DEPLOY_ROOT") BACKEND_HOST=$(quote "$BACKEND_HOST") BACKEND_PORT=$(quote "$BACKEND_PORT") FRONTEND_HOST=$(quote "$FRONTEND_HOST") FRONTEND_PORT=$(quote "$FRONTEND_PORT") bash -s -- $(quote "$ACTION")" \
